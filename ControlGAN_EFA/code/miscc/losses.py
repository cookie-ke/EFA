...
def discriminator_loss(netD, real_imgs, fake_imgs, conditions,
                       real_labels, fake_labels, words_embs, cap_lens, image_encoder, class_ids,
                        w_words_embs, wrong_caps_len, wrong_cls_id, label, index):
    # if index == 2:
    ...
    # ================= new add cls_loss
    # if index == 2:
    out_cls = netD(real_imgs)[1]
    # d_loss_real = - torch.mean(out_src)
    d_loss_cls = F.binary_cross_entropy_with_logits(out_cls, label, size_average=False) / out_cls.size(
        0) * 0.1

    # Loss functions for fake images and captions_attr
    out_cls_fake = netD(fake_imgs)[1]
    # d_loss_real = - torch.mean(out_src)
    d_loss_cls_fake = F.binary_cross_entropy_with_logits(out_cls_fake, label, size_average=False) / out_cls_fake.size(
        0) * 0.1
    errD += (d_loss_cls + d_loss_cls_fake)
    return errD, d_loss_cls, d_loss_cls_fake
    # return errD


def generator_loss(netsD, image_encoder, fake_imgs, real_labels,
                   words_embs, sent_emb, match_labels,
                   cap_lens, class_ids, style_loss, real_imgs, label):
    numDs = len(netsD)
    ...
    for i in range(numDs):
        ...
        # if i == 2:
        out_cls = netsD[i](real_imgs[i])[1]
        # d_loss_real = - torch.mean(out_src)
        d_loss_cls = F.binary_cross_entropy_with_logits(out_cls, label, size_average=False) / out_cls.size(
            0) * 0.1

        # Loss functions for fake images and captions_attr
        out_cls_fake = netsD[i](fake_imgs[i])[1]
        # d_loss_real = - torch.mean(out_src)
        d_loss_cls_fake = F.binary_cross_entropy_with_logits(out_cls_fake, label,
                                                             size_average=False) / out_cls_fake.size(
            0) * 0.1
        errG_total += (d_loss_cls + d_loss_cls_fake)
